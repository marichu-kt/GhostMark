...
export function Input({ label, helpText, error, id, className, ...props }: InputProps) {
  ...
  return (
    <label className="grid gap-1.5 text-sm text-steel-100" htmlFor={inputId}>
      <span className="font-medium">{label}</span>
      <input
        id={inputId}
        className={classNames(
          "min-h-10 rounded-md border border-graphite-700 bg-graphite-950 px-3 py-2 text-sm text-white placeholder:text-steel-500",
          className,
        )}
        {...props}
      />
